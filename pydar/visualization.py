...
# Convenience function for creating animated visualizations
def create_animated_visualization(states: List[Dict], config: Optional[VisualizationConfig] = None) -> go.Figure:
    """Create an animated figure from a list of states."""
    visualizer = Radar3DVisualizer(config)
    
    # Create frames
    frames = []
    for i, state in enumerate(states):
        frame_fig = visualizer.create_figure_from_state(state)
        frames.append(go.Frame(data=frame_fig.data, name=str(i)))
    
    # Create initial figure
    fig = visualizer.create_figure_from_state(states[0])
    fig.frames = frames
    ...
    fig.update_layout(
        ...
        sliders=[{
            'steps': [{'args': [[str(i)]], 'label': f'{i}', 'method': 'animate'} 
